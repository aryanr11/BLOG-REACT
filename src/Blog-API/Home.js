...
const Home = () => {
  ...
  const getBlog = () => {
    axios.get('https://blog-api-n83g.onrender.com/blog/find')
      .then((res) => {
          ...
          <Row className="  ">
            {
              blog.map((el, index) => {
                return <div className="col-lg-4 position-relative img-hov" key={index}>
                  <div className=''>
                    <img width="100%" className='' onClick={() => card(el._id)} src={'https://blog-api-n83g.onrender.com/images/' + el.image} alt="" />
                  </div>
